...
LRESULT CALLBACK ProcessWindow(HWND hWnd, UINT uiMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_LBUTTONDOWN:
	{
		...
		if (sysCtx && sysCtx->mouseDownFunc)
			sysCtx->mouseDownFunc((int)point.x, (int)point.y);
	}
	break;
	...
	case WM_LBUTTONUP:
	{
		...
		if (sysCtx && sysCtx->mouseUpFunc)
			sysCtx->mouseUpFunc( (int)point.x, (int)point.y);
	}
	break;
	...
	case WM_MOUSEMOVE:
		if (wParam & MK_LBUTTON)
		{
			...
			point = MAKEPOINTS(lParam);
			if (sysCtx && sysCtx->mouseMoveFunc)
				sysCtx->mouseMoveFunc((int)point.x, (int)point.y);
		}
		//break;
	case WM_KEYDOWN:
	{
		if (sysCtx && sysCtx->keyFunc && uiMsg == WM_KEYDOWN)
